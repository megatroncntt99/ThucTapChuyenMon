...
public class MainActivity extends AppCompatActivity implements TabLayout.OnTabSelectedListener {
    ...
    protected void onStop() {
        super.onStop();
        database.SetData("DELETE FROM SongPosition");
        database.SetData("Insert into SongPosition values('"+position+"')");
    ...
    private void loadDataSong() {
        ...
        if(songArrayList.size()!=1){
            Cursor cursor1 = database.GetData("Select * from SongPosition");
            while (cursor1.moveToNext()) {
                position=cursor1.getInt(0);
            }
        }
    ...
    private void createDatabase() {
        ...
        database.SetData("create table if not exists SongLike(IdSong integer )");
        database.SetData("create table if not exists PlaylistLike(IdPlaylist integer )");
        database.SetData("create table if not exists CategoryLike(IdCategory integer )");
        database.SetData("create table if not exists AlbumLike(IdAlbum integer )");
        ...
        database.SetData("create table if not exists SongNew(idSong nvarchar(255) primary key,nameSong nvarchar(255)," +
                " imgSong nvarchar(255),singer nvarchar(255),linkSong nvarchar(255),likeSong nvarchar(255))");
        database.SetData("create table if not exists SongPosition(position integer)");
    }
